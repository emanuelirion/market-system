...
import com.emanuel.mapper.ShiftReportMapper;
import com.emanuel.modal.*;
import com.emanuel.payload.dto.ShiftReportDTO;
import com.emanuel.repository.*;
import com.emanuel.services.ShiftReportService;
...
public class ShiftReportServiceImpi implements ShiftReportService {
    ...
    public ShiftReportDTO startShift() throws Exception {
       ...
       LocalDateTime shiftStart = LocalDateTime.now();
        ...
        LocalDateTime endfDay = shiftStart.withHour(23).withMinute(59).withSecond(59);

        Optional<ShiftReport> existing = shiftReportRepository.findByCashierAndShiftStartBetween(currentUser, startOfDay, endfDay);
    ...
    public ShiftReportDTO endShift(Long shiftReportId, LocalDateTime shiftEnd) throws Exception {
        User currentUser = userService.getCurrentUser();
        ShiftReport shiftReport = shiftReportRepository.findTopByCashierAndShiftEndIsNullOrderByShiftStartDesc(currentUser).orElseThrow(() -> new Exception("ShiftReport not found"));
        ...
        double netSale=totalSales-totalOrders;

        shiftReport.setTotalRefunds(totalRefunds);
        shiftReport.setTotalSales(totalSales);
        shiftReport.setTotalOrders(totalOrders);
        shiftReport.setNetSales(netSale);
        shiftReport.setRecentOrders(getRecentOrders(orders));
        shiftReport.setTotalSellingProducts(getTopSellingProducts(orders));
    ...
    @Override
    public List<ShiftReportDTO> getShiftsReportsByBranchId(Long branchId) throws Exception {
    ...
    public ShiftReportDTO getCurrentShiftProgress(Long cashierId) throws Exception {
        ...
        ShiftReport shiftReport = shiftReportRepository.findTopByCashierAndShiftEndIsNullOrderByShiftStartDesc(user).orElseThrow(() -> new Exception("no active shift found for cashier"));
    ...
    public ShiftReportDTO getShiftByCashierAndDate(Long cashierId, LocalDateTime date) throws Exception {
        User cashier = userRepository.findById(cashierId).orElseThrow(() -> new Exception("cashier not found with given date"));
